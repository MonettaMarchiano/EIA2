...
namespace Aufgabe2 {
    ...
    function init(): void { // void = ohne spezifische Rückgabe
        ...
        let cards = promptCards(); //promptCards (function) wird let cards zugwiesen --> das was bei prompt rauskommt, ist der Wert von Cards
    ...
    function createGame(_cards: number): void { //_cards:number ist Parameter der übergeben wird
        ...
        let main: HTMLElement = document.createElement("main"); //eine Methode als Wert zugewiesen (document.kreiere ein Element("welcherArt")
    ...
    //_____________________________________________________________________________________________________________________     
    function createGameArea(_cards: number, _main: HTMLElement): void {
        ...
        shuffleArray(cardContentAll); //Funktion, weiter unten, wird aufgerufen (Befehl mit dem Parameter Array zum Mischen)
        ...
        let cardContentHand: string[] = cardContentAll.slice(0, _cards); //ist vom Typ Array mit Strings(Liste von Strings); durch slice wird großes array nicht angerührt
        
        // selbes prinzip
        // sprich die 6. karte (cCH.[5])
        // DO NOT CHANGE, das ist die karte, die obligatorisch offen liegt
        let cardContentOpen: string[] = cardContentAll.slice(_cards, (_cards+1));
        console.log(cardContentOpen);
        
        // selbes prinzip
        // zieht den rest der karte raus, also inkl. 7. karte bis zum schluss (cCH.[6],cCH.[7],..[...]..cCH.[107],cCH.[108])
        // "simuliert" den stack
        let cardContentStack: string[] = cardContentAll.slice((_cards+1), cardContentAll.length);
        console.log(cardContentStack);
        
        // Stack
        // AufnahmeStapel neue Karten
        let stackSec: HTMLElement = document.createElement("section");
        stackSec.id = "stackSec";   //falls es für css gebraucht, für bspw. spätere Spielogik
        _main.appendChild(stackSec); 
        ...
        createCard("img/unoBack.png", stackSec);
    ...
    function createCard(_src: string, _parent: HTMLElement): void { //_parent ist Elternteil von jeweiliger Karte (HandSec, StackSec, DiscardSec)
